...
const configFromFile = (file: string | undefined) => {
  ...
  // If documentSelector is not defined, it may be defaulted elsewhere.
  return {};
};
...
export class PluginLocator {
  ...
  static async all(
    ...
    return flatten(
      await Promise.all([
        ...
        this.builtin(sysl, extensionPath, options),
        options?.workspaceConfig?.plugins?.fetchFromNetwork
          ? this.networkPlugins(sysl, globalStoragePath, remoteUrl, options).catch((err) => {
  ...
  static async localPlugins(
    ...
    async function transformPlugins(dir: string): Promise<PluginConfig[]> {
      const scriptsPath = path.join(dir, syslMetaDir, "diagram_renderers");
    ...
    async function lspPlugins(dir: string): Promise<PluginConfig[]> {
      const scriptsPath = path.join(dir, syslMetaDir, "plugins", "lsp");
