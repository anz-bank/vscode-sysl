      ...
      <div className={classes.root}>
        <TabContext value={value}>
          ...
          { data.map((diagramData, index) => {
            ...
            return <Tab key={index} classes={{root:classes.tab}} label={label} value={index.toString()}></Tab>;
          })}
          </TabList>
          { data.map((diagramData, index) => {
            const key = diagramData.templates ? "custom" : "";
            return <TabPanel classes={{root:classes.tabPanel}} value={index.toString()} key={index}>
                <Diagram
                  diagramKey={key}
                  template={diagramData.templates}
                  nodes={diagramData.nodes}
