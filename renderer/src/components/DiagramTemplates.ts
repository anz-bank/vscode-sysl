...
import { TemplateData, TemplateEdgeData, TemplateGroupData, TemplateNodeData, TemplateSectionData, TemplateShadowData, TemplateShape, TemplateTextBlock } from "./DiagramTypes";
...
export default function DiagramTemplate(props?: TemplateData) {
	...
	const linkTemplate = $(go.Link,
		...
		$(go.Shape, new go.Binding("fill", "color"), new go.Binding("stroke", "color"), { toArrow: "Standard" })
	);
...
function getCustomGroupTemplate(groupTemplate: TemplateGroupData): go.Group {
	return $(go.Group, "Position",
	new go.Binding("isSubGraphExpanded", "expanded").makeTwoWay(),
	groupTemplate.shadow ? getShadowStyle(groupTemplate.shadow) : {},

	// Collapsed group view
	groupTemplate.collapsed ? $(go.Panel, "Spot",
		new go.Binding("visible", "expanded", (e) => { return !e }),
	...
	// Expanded group view
	groupTemplate.expanded ? $(go.Panel, "Spot",
...
function getSectionStyles(sections: TemplateSectionData[]): go.ObjectData[] {
	return sections.map((section: TemplateSectionData) => $(go.Panel,
		section.panelType,
		section.background
			? isColor(section.background)
				? { background: section.background}
				: new go.Binding("background", section.background)
			: {"background": "#FF00FF"},
		section.mainPanel ? { isPanelMain: section.mainPanel } : {},
		section.minSize ? { minSize: new go.Size(section.minSize.x, section.minSize.y) } : {},
		section.desiredSize ? { desiredSize: new go.Size(section.desiredSize.x, section.desiredSize.y) } : {},
...
function getShapeStyle(shape: TemplateShape): go.Shape {
	return $(go.Shape,
		...
		shape.fill
			? isColor(shape.fill)
				? { fill: shape.fill}
				: new go.Binding("fill", shape.fill)
			: {fill: "#FF00FF"},
	);
}
...
function getTextBlockStyle(text: TemplateTextBlock): go.TextBlock {
	return $(go.TextBlock,
		...
		{ font: text.font ?? "" },
		text.segmentOffset ? { segmentOffset: new go.Point(text.segmentOffset.x, text.segmentOffset.y) } : {},
		text.stretch ? { stretch: go.GraphObject.Fill } : {},
		text.margin ? { margin: go.Margin.parse(text.margin) } : {},
		{ stroke: text.stroke ?? "black"},
