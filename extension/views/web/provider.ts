...
export class CustomEditorViewFactory {
  ...
  async create<T, D>(key: ViewKey, kind: ViewKind, initialModel?: T): Promise<View<T, D>> {
    if (kind === "html") {
  ...
  private async open<T, D>(key: ViewKey, providerType: string, model?: T): Promise<View<T, D>> {
